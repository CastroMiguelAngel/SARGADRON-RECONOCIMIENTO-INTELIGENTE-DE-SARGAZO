...
def funcion_dron(ruta_guarda, fotos):
    ...
    mensaje.config(text=f"El dron esta despegando...", font=("Helvetica", 22))
...
def iniciar_oficial():
    ...
    mensaje.config(text=f"Señal de procesamiento completado enviada.", font=("Helvetica", 22))
    mensaje.place(x=680, y=300)
...
def iniciar_prueba():
    ...
    mensaje.config(text=f"Señal de procesamiento completado enviada y socket cerrado con exito", font=("Helvetica", 22))
    mensaje.place(x=680, y=300)

def mostrar_mensaje():
    num_imagenes = seleccion.get()
    ...
    # Actualizar el mensaje
    mensaje.config(text=f"El dron comenzará a tomar {num_imagenes} imágenes.", font=("Helvetica", 22))
    mensaje.place(x=680, y=300)  # Ajusta de coordenadas
    return num_imagenes
...
boton_comenzar.place(x=750, y=450)  # Ajusta de coordenadas

# Botón "Comenzar"
boton_comenzar_prueba = ttk.Button(cuadro, text="Comenzar prueba", command=iniciar_prueba, width=15, style='My.TButton')
boton_comenzar_prueba.place(x=950, y=450)  # Ajusta de coordenadas
